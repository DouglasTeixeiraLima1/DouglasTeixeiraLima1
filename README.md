<div align="center">

# 👨‍💻 Douglas Teixeira Lima

### Estudante de Informática para Negócios | FATEC Mauá

</div>


# 👋 Olá, eu sou Douglas Teixeira Lima

🎓 Estudante de **Informática para Negócios** na FATEC Mauá
💼 Estagiário de TI
💻 Desenvolvedor em formação com foco em C#, Python e Banco de Dados
🚀 Interessado em Desenvolvimento de Software, Inteligência Artificial e Empreendedorismo Tecnológico

---

## 📚 Atualmente estudando

* Estruturas de Dados
* Engenharia de Software
* Banco de Dados
* Programação Orientada a Objetos
* Git e GitHub
* Python
* C#

---

## 🛠️ Tecnologias

<div>
  <img height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" />
  <img height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" />
  <img height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/microsoftsqlserver/microsoftsqlserver-plain.svg" />
  <img height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" />
  <img height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" />
  <img height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/visualstudio/visualstudio-plain.svg" />
</div>

---

## 🚀 Projeto em Destaque

### 💇 HairUp ERP

Sistema ERP para gestão de salões de beleza desenvolvido em **C#** e **SQL Server**.

### Funcionalidades

✅ Cadastro de clientes
✅ Cadastro de profissionais
✅ Cadastro de serviços
✅ Gerenciamento de agenda
✅ Controle de agendamentos
✅ Operações CRUD completas
✅ Banco de dados relacional

### Características Técnicas

* 17 telas funcionais
* SQL Server
* Relacionamentos com chaves estrangeiras
* Integração entre clientes, profissionais e serviços
* Controle de horários e agendamentos

---

## 🎯 Objetivos

Busco evoluir como desenvolvedor de software, aprofundar meus conhecimentos em programação e construir soluções tecnológicas que resolvam problemas reais.

Tenho interesse especial em:

* Desenvolvimento de Sistemas
* Inteligência Artificial
* Engenharia de Software
* Estruturas de Dados
* Startups e Inovação

---

## 🌐 Contato

🔗 LinkedIn
[www.linkedin.com/in/douglastlima](http://www.linkedin.com/in/douglastlima)

🔗 GitHub
https://github.com/DouglasTeixeiraLima1
